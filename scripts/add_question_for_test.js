...
function findSubjectAndInsert() {
  db.get(
    "SELECT id, name, code FROM subjects WHERE name LIKE '%Math%' OR name LIKE '%math%' OR name LIKE '%Mathematics%' LIMIT 1",
    [],
    (err, row) => {
      if (err) {
        console.error('ERR_SUBJECT', err);
        process.exit(1);
      }
      if (!row) {
