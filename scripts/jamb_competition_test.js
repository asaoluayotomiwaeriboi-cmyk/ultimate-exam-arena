(async () => {
  ...
  try {
    ...
    if (!process.env.ADMIN_PASSWORD)
      console.warn(
        'Using fallback admin password in jamb_competition_test.js — set ADMIN_PASSWORD env for tests.'
      );
    ...
    r = await fetchJson(base + '/api/admin/competitions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${adminToken}` },
      body: JSON.stringify({ subjectCode: 'JAMB', durationMinutes: 30 }),
    });
    ...
    r = await fetchJson(base + '/api/exams/start', {
      ...
      body: JSON.stringify({ subjectCode: 'JAMB' }),
    });
    log('startExam:', r);
    ...
    if (questions && questions.length) {
      ...
      await fetchJson(base + '/api/exams/answer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${studentToken}` },
        body: JSON.stringify({
          sessionId,
          questionId: q0.id,
          answer: (q0.choices && q0.choices[0]) || 'N/A',
        }),
      });
    }
