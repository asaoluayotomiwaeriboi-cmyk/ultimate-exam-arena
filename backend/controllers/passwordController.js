      ...
      return new Promise((resolve, reject) => {
        ...
        `, [examType], function(err) {
          ...
          db.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, 1)
          `, [passwordId, examType, password, passwordHash, createdAt, expiresAt], function(err) {
            if (err) return reject(err);
  ...
  sendPasswordToStudents: async (passwordId, examType = 'UTME') => {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        `, [passwordId, examType], (err, passwordRow) => {
          if (err) return reject(err);
          ...
          db.all(`SELECT id, email, name FROM users WHERE role = 'student'`, [], async (err, students) => {
            if (err) return reject(err);
      ...
      return new Promise((resolve, reject) => {
        ...
        `, [passwordId], (err, rows) => {
          ...
          const distributions = (rows || []).map(row => ({
            ...
            lastAttemptAt: row.lastAttemptAt ? new Date(row.lastAttemptAt * 1000).toISOString() : null
          }));
